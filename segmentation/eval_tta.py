...
def main():
    ...
    logger.info(f"Start testing with TTA {len(test_loader)} images in {dataset} dataset")
    ...
    for i, pack in tqdm.tqdm(enumerate(test_loader, start=1)):
        ...
        for j in range(3):
            if j == 0:
                ...
                out = nn.functional.interpolate(out, original_size, mode='bilinear', align_corners=True)
                out = out.data.sigmoid().cpu().numpy().squeeze()
                out = (out - out.min()) / (out.max() - out.min() + 1e-8)
            if j == 1:
                out_v = model(torch.flip(image, dims=(2, )))
                out_v = F.interpolate(out_v, original_size, mode='bilinear', align_corners=True)
                out_v = nn.functional.interpolate(out_v, original_size, mode='bilinear', align_corners=True)
            ...
            if j == 2:
                ...
                out_h = F.interpolate(out_h, original_size, mode='bilinear', align_corners=True)
                out_h = nn.functional.interpolate(out_h, original_size, mode='bilinear', align_corners=True)
