...
def main():
    ...
    for i, pack in tqdm.tqdm(enumerate(test_loader, start=1)):
        ...
        if batch_size ==  1:
            rles.append(rle_encode(res))
            df.loc[i, 'task'] = 0
            df.loc[i, 'id'] = filename[0]
            df.loc[i, 'label'] = rles[0]
        else:
            for mask in res:
                rles.append(rle_encode(mask))
            df.loc[i: i* batch_size, 'task'] = 0
            df.loc[i: i* batch_size, 'id'] = filename
            df.loc[i: i* batch_size, 'label'] = rles
    df.to_csv(f'{csv_save}/sub.csv', index=False)
    return df
